...
public class WholePage extends RowComposite {
  ...
  public void onCtrlBtnClicked(MouseEvent arg) throws TwitterException {
    ...
    if (control != null) {
      ...
      else if (title.equals("Post Status")){
    	  String status = m_homePage.getStatus();
    	  if(status.length() > 0 && status.length()<=140){
    		  TwitterControl.getTwitter().updateStatus(status);
    		  long t1 = System.currentTimeMillis();
    		  Shell parent = (Shell)this.getParent().getParent();
    		  while(System.currentTimeMillis()-t1<=5000);
    		  gotoHomePage();
   		      final Shell shell = new Shell(parent.getDisplay(),SWT.NONE);
    		// center the initial window
    		  //Rectangle full = parent.getDisplay().getPrimaryMonitor().getBounds();
    		  //Rectangle rect = shell.getBounds();
    		  shell.setLocation(500,300);  
    		  
    		  shell.setBackgroundImage(Resources.POSTED_INFO_IMG);
    		  shell.setText("Your Tweet Was Posted!");
    		  shell.setSize(526, 40);
    		  shell.open();
    		  layout();
  ...
  private void gotoUserPage(User user, String itemList) throws TwitterException {
    ...
    m_userPage = new UserPage(this, user, itemList, m_width, 
        m_height - 40, 300, 98, 520, 260, "onNameLinkClicked", this,false);
