...
public class FriendView extends RowComposite {
  ...
  public FriendView(Composite parentView, User friend, int width, String nameClkHandler, Object handlerCallee,boolean  isFollowedByMe) {
...
/*    if(!m_isFollowedByMe){
    ...
    Composite followcomposite = new Composite(rightFrame, SWT.NONE);        
...
//    Utils.cutRoundCorner(m_iconImage, true, true, true, true);
  ...
  public void followfunction(MouseEvent arg) {
		 ...
		 {
			 try {
					TwitterControl.getTwitter().createFriendship(m_friend.getScreenName());
				    TwitterControl.refreshFriendShip();
			 ...
			 try {
					TwitterControl.getTwitter().destroyFriendship(m_friend.getScreenName());
				    TwitterControl.refreshFriendShip();
				} catch (TwitterException e) {
					Shell errShell = new Shell();
					errShell.setSize(300, 172);
